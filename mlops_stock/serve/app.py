...
async def predict_next_day_price():
    ...
    try:
        ...
        prediction = predict_next_day(
            features_path=_features_path, model_path=_model_path
        )

        return PredictionResponse(
            prediction=float(prediction), message="Prediction successful"
        )
...
async def predict_custom(request: PredictionRequest):
    ...
    try:
        if _model is None:
            if not Path(_model_path).exists():
                raise HTTPException(
                    status_code=404, detail=f"Model file not found at {_model_path}"
                )
            _model = load_model(_model_path)
        ...
        return PredictionResponse(
            prediction=float(prediction), message="Custom prediction successful"
        )
