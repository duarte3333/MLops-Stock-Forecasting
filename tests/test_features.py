...
def test_features_shape(sample_stock_data):
    ...
    assert (
        df_features.shape[0] == sample_stock_data.shape[0]
    ), "Number of rows should match original data"

    # Should have more columns than original
    assert (
        df_features.shape[1] > sample_stock_data.shape[1]
...
def test_build_features_saves_file(sample_stock_data):
    ...
    df_features = build_features(
        input_path=test_input_path, output_path=test_output_path, dropna=True
    )
...
def test_features_dropna(sample_stock_data):
    ...
    assert (
        df_no_na[numeric_cols].isna().sum().sum() == 0
